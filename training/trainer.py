...
from training.utils.val_utils import save_result
...
class Trainer:
    ...
    def __init__(
        ...
        *,  # the order of these args can change at any time, so they are keyword-only
        ...
        is_draw_result: Optional = False,
    ):
        self.is_draw_result = is_draw_result
    ...
    def _step(
        ...
        outputs = model(batch)
        targets = batch.masks
        categories = batch.categories if hasattr(batch, "categories") else None
        batch_size = len(batch.img_batch)

        key = batch.dict_key  # key for dataset
        loss = self.loss[key](outputs, targets)
        if (self.mode == Phase.VAL or self.mode == Phase.TEST) and self.is_draw_result:
            save_result(outputs, targets, batch.metadata.unique_objects_identifier)
    ...
    def _setup_dataloaders(self):
        ...
        self.tokenizer = instantiate(self.data_conf['tokenizer'])
        collate_fn_train = instantiate(
            self.data_conf['collate_fn'],
            tokenizer=self.tokenizer,
            max_num_objects_total=self.data_conf['max_num_objects_total']
        )
        collate_fn_val = instantiate(self.data_conf['collate_fn'], tokenizer=self.tokenizer)

        if self.mode in ["train", "val", "test"]:
            self.test_dataset = instantiate(self.data_conf.get(Phase.TEST, None), collate_fn=collate_fn_val)

        if self.mode in ["train", "val"]:
            self.val_dataset = instantiate(self.data_conf.get(Phase.VAL, None), collate_fn=collate_fn_val)

        if self.mode in ["train", "train_only"]:
            self.train_dataset = instantiate(self.data_conf['train'], collate_fn=collate_fn_train)
    ...
    def val_epoch(self, val_loader, phase):
        ...
        loss_mts = OrderedDict(
            [(name, AverageMeter(name, self.device, ":.4e")) for name in loss_names]
        )
        extra_loss_mts = {}
        ...
        progress = ProgressMeter(
            ...
            prefix="Test Epoch: [{}]".format(self.epoch) if phase == Phase.TEST
                else "Val Epoch: [{}]".format(self.epoch),
        )
                    ...
                    for phase, model in zip(curr_phases, curr_models):
                        ...
                        for k, v in extra_losses.items():
                            if k not in extra_loss_mts:
                                extra_loss_mts[k] = AverageMeter(k, self.device, ":.4e")
    ...
    def train_epoch(self, train_loader):
        ...
        # Model training loop
        if isinstance(self.freeze_image_encoder, int) and self.epoch == self.freeze_image_encoder:
            if isinstance(self.model, DistributedDataParallel):
                for p in self.model.module.image_encoder.parameters():
                    p.requires_grad = True
            else:
                for p in self.model.module.image_encoder.parameters():
                    p.requires_grad = True
        if isinstance(self.freeze_text_encoder, int) and self.freeze_text_encoder == self.epoch:
            if isinstance(self.model, DistributedDataParallel):
                for param in self.model.module.text_encoder.parameters():
        ...
        for data_iter, batch in enumerate(train_loader):
            ...
            # Catching NaN/Inf errors in the loss
                ...
                logging.error("OutOfMemoryError: flat_img_batch size: {}, flat_obj_to_img_idx: {}"
                              .format(batch.flat_img_batch.size(), batch.flat_obj_to_img_idx.cpu().tolist()))
                raise e
    ...
    def _setup_components(self):
        ...
        self.freeze_image_encoder = self.model_conf.get("freeze_image_encoder", False)
        self.freeze_text_encoder = self.model_conf.get("freeze_text_encoder", False)
        ...
        if self.meters_conf:
            ...
            for _, phase_meters in self.meters.items():
                for _, key_meters in phase_meters.items():
