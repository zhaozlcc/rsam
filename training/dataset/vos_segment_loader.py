...
class MyLazySegments(LazySegments):
    def __init__(self):
        super().__init__()
        self.descriptions = {}
        self.categories = {}
    ...
    def get_description(self, key):
        return self.preprocessing_caption(self.descriptions[key])

    def preprocessing_caption(self, caption: str, max_length: int = 30):
        caption = caption.lower()
        caption = caption.strip().replace('-', ' ').replace('/', ' ').replace('<person>', 'person')
        caption = re.sub(
            r"\s{2,}",
            ' ',
            caption,
...
class MySegmentLoader:
    def __init__(
        self,
        video_mask_path,
        negative_mask_paths=None,
        mask_area_frac_thresh=1.1,
        video_frame_path=None,
        uncertain_iou=-1,
        ...
        if negative_sample_size is None:
            negative_sample_size = [800, 800]
        if negative_mask_paths is None:
            negative_mask_paths = []
        try:
            with open(video_mask_path, "r") as f:
                self.frame_annots = json.load(f)
        except:
            print(f"{video_mask_path} open failed.")
            raise
        ...
        for frame_annot in self.frame_annots:
            ...
            if (
                ...
                continue
            rle_masks.append(frame_annot["segmentation"])
            descriptions.append(frame_annot["description"])
            description_names.append(frame_annot["name"])

        try:
            for negative_mask_path in negative_mask_paths:
                with open(negative_mask_path, "r") as f:
                    negative_annots = json.load(f)
                negative_annots = negative_annots["annotations"]
                for negative_annot in negative_annots:
                    if negative_annot["name"] not in description_names:
                        rle_masks.append({"size": negative_sample_size, "counts": b'PPac0'})
                        descriptions.append(negative_annot["description"])
        except:
            print(f"Read negative sample error: {negative_mask_path}")
            raise

        self.segments = MyLazySegments()
        for i, (rle, description, category) in enumerate(zip(rle_masks, descriptions, description_names)):
            self.segments[i] = rle
            self.segments.descriptions[i] = description
            self.segments.categories[i] = category

    def load(self, frame_idx):
        return self.segments


class _MySegmentLoader:
    def __init__(
        self,
        video_mask_path,
        negative_mask_paths=None,
        ...
        description_names = []
        for frame_annot in self.frame_annots:
            if isinstance(frame_annot['predicted_iou'], list):
                idx = np.argmax(frame_annot['predicted_iou'])
                if not frame_annot["area"][idx] > 0:
                    continue
                if ("uncertain_iou" in frame_annot) and (
                        frame_annot["uncertain_iou"][idx] < uncertain_iou
                ):
                    # uncertain_iou is stability score
                    continue
                if (
                        mask_area_frac_thresh <= 1.0
                        and (frame_annot["area"][idx] / area) >= mask_area_frac_thresh
                ):
                    continue
                rle_masks.append(frame_annot["segmentation"][idx])
            else:
                if not frame_annot["area"] > 0:
                    continue
                if ("uncertain_iou" in frame_annot) and (
        ...
        self.segments = MyLazySegments()
        for i, (rle, description) in enumerate(zip(rle_masks, descriptions)):
            self.segments[i] = rle
            self.segments.descriptions[i] = description
