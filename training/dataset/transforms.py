...
class RandomBlackBoxesAPI:
    def __init__(self, prob=0.15, min_num=20, max_num=60, size=32):
        self.prob = prob
    ...
    def __call__(self, datapoint, **kwargs):
        ...
        for frame in datapoint.frames:
            ...
            for k in range(num):
                y, x = random.randint(0, img_size - self.size), random.randint(0, img_size - self.size)
                image[:, y:y + self.size, x: x + self.size] = 0
