...
class CkptRsamInitKernel:
    def __init__(self):
        self.to_remove = {'sam_mask_decoder.'}
        self.to_reserve = {'image_encoder.', 'text_encoder.'}

    def __call__(self, state_dict: Dict, model: nn.Module):
        state_dict = self.remove_mattention_name(state_dict, model)
        state_dict = self.replace_bert_to_textencoder(state_dict)
        state_dict = self.remove(state_dict)
        state_dict = self.reserve(state_dict)
        return state_dict

    @staticmethod
    def replace_bert_to_textencoder(state_dict: Dict):
        fix = {'bert.': 'text_encoder.', 'gamma': 'weight', 'beta': 'bias'}
        new_state_dict = {}
        for k, v in state_dict.items():
            if k.startswith('cls.') or k.startswith('bert.pooler.'):
                continue
            if not k.startswith('bert.'):
                new_state_dict[k] = v
                continue
            for f_k, f_v in fix.items():
                if f_k in k:
                    k = k.replace(f_k, f_v)
            new_state_dict[k] = v
    ...
    @staticmethod
    def remove_mattention_name(state_dict, model):
        model_para_name = model.state_dict().keys()
        rewrite_dict = {}
        for para in model_para_name:
            if 'raw_attention.' in para:
                new_name = para
                old_name = para.replace('raw_attention.', '')
                rewrite_dict[old_name] = new_name

        new_state_dict = {}
        for key, value in state_dict.items():
            if key in rewrite_dict:
                key = rewrite_dict[key]
            new_state_dict[key] = value

        return new_state_dict

    def remove(self, state_dict):
        new_state_dict = {}
        for key, value in state_dict.items():
    ...
    def reserve(self, state_dict):
        if self.to_reserve is None or len(self.to_reserve) == 0:
            return state_dict
        new_state_dict = {}
        for key, value in state_dict.items():
            for pattern in self.to_reserve:
                if key.startswith(pattern):
                    new_state_dict[key] = value
...
def load_state_dict_into_model(
    ...
    if checkpoint_kernels is not None:
        for f in checkpoint_kernels:
            if inspect.signature(f).parameters.__contains__('model'):
                state_dict = f(state_dict=state_dict, model=model)
            else:
...
def load_checkpoints(
    ...
    for checkpoint in checkpoints:
        # Load the checkpoint on CPU to avoid GPU mem spike.
        with g_pathmgr.open(checkpoint['checkpoint_path'], "rb") as f:
            checkpoint_weight = torch.load(f, map_location=map_location, weights_only=False)
        combined_weights.update(get_state_dict(checkpoint_weight, checkpoint.get('ckpt_state_dict_keys', tuple())))
