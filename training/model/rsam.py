...
from rsam.modeling.sam2_base import SAM2Base
from rsam.modeling.sam2_utils import (
    get_1d_sine_pe,
    get_next_point,
    sample_box_points,
...
from rsam.utils.misc import concat_points
...
class RSAM(SAM2Base):
    def __init__(
        ...
        prob_to_use_descript_input_for_train=0.0,
        prob_to_use_descript_input_for_eval=0.0,
        ...
        **kwargs,
        ...
        self.text_proj = torch.nn.Linear(self.text_encoder.config.hidden_size, self.sam_prompt_embed_dim)
        ...
        self.prob_to_use_descript_input_for_train = prob_to_use_descript_input_for_train
        self.prob_to_use_pt_input_for_eval = prob_to_use_pt_input_for_eval
        self.prob_to_use_box_input_for_eval = prob_to_use_box_input_for_eval
        self.prob_to_use_descript_input_for_eval = prob_to_use_descript_input_for_eval
        if prob_to_use_pt_input_for_train > 0:
            logging.info(
                f"Training with points (sampled from masks) as inputs with p={prob_to_use_pt_input_for_train}"
            )
            assert num_frames_to_correct_for_train >= num_init_cond_frames_for_train

        if prob_to_use_pt_input_for_eval > 0:
        ...
        if compile_text_encoder:
            ...
            print(
                "Text encoder compilation is enabled. First forward pass will be slow."
            )
    ...
    def prepare_prompt_inputs(self, backbone_out, input, start_frame_idx=0):
        ...
        backbone_out["descriptions"] = input.descriptions

        # Randomly decide whether to use point inputs or mask inputs
        if self.training:
            prob_to_use_pt_input = self.prob_to_use_pt_input_for_train
            prob_to_use_box_input = self.prob_to_use_box_input_for_train
            prob_to_use_descript_input = self.prob_to_use_descript_input_for_train
            ...
            prob_to_use_descript_input = self.prob_to_use_descript_input_for_eval
        ...
        backbone_out["descript_inputs_per_frame"] = {}
        for t in init_cond_frames:
            use_descript_input = self.rng.random() < prob_to_use_descript_input
            if use_descript_input:
                input_ids = input.descriptions.input_ids[t]
                attention_mask = input.descriptions.attention_mask[t]
                descript_feat = self.text_proj(self.text_encoder(input_ids, attention_mask=attention_mask))
                backbone_out["descript_inputs_per_frame"][t] = descript_feat
    ...
    def forward_tracking(
        ...
        for stage_id in processing_order:
            ...
            current_out = self.track_step(
                ...
                description_inputs=backbone_out["descript_inputs_per_frame"].get(stage_id, None),
        ...
        for _ in range(self.num_correction_pt_per_frame):
            ...
            if self.use_act_ckpt_iterative_pt_sampling and not multimask_output:
                ...
                sam_outputs = self._forward_sam_heads(
                    ...
                    point_inputs=point_inputs,
                    mask_inputs=mask_inputs,
                    description_inputs=description_inputs,
                    high_res_features=high_res_features,
                    multimask_output=multimask_output,
                )
            (
