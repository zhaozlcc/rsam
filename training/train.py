...
def main(args) -> None:
    ...
    os.environ['SYNC_TIMESTAMP'] = timestamp
    timestamp = os.environ.get('SYNC_TIMESTAMP')
    if args.log_dir is not None:
        cfg.launcher.experiment_log_dir = os.path.join(
            os.getcwd(), "rsam_logs", args.log_dir
        )
    elif cfg.launcher.experiment_log_dir is None:
        cfg.launcher.experiment_log_dir = os.path.join(
            os.getcwd(), "rsam_logs", f"{timestamp}", args.config
        )
...
if __name__ == "__main__":
    initialize_config_module("rsam", version_base="1.2")
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="path to config file (e.g. configs/rsam_b+_b.yaml)",
    )
    parser.add_argument(
        "--use-cluster",
        type=int,
